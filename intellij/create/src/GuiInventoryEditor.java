...
public class GuiInventoryEditor extends JFrame {
    ...
    private void add() {
        inv.addItem(Popup.dropDown(StaticStuff.PROJECT_NAME, "Select an item UID", Manager.getStringArrayItems()).replaceAll(".+ - ([^-]+)", "$1"));
        ta_items.setText(inv.getInventoryString());
    }

    private void remove() {
        inv.removeItem(Popup.dropDown(StaticStuff.PROJECT_NAME, "Select an item UID", Manager.getStringArrayItems()).replaceAll(".+ - ([^-]+)", "$1"));
        ta_items.setText(inv.getInventoryString());
    }
