...
public class ColorObject extends Entity{
    ...
    public ColorObject(String[] fileInput){
        ...
        }catch(Exception e){Popup.error(StaticStuff.projectName, "Color '"+name+"' contains invalid data.");}
    }
    ...
    public int additionalRefactor(String find, String replace){
        int occ = 0;
        occ += StaticStuff.countOccurrences(r+"", find); r = Integer.parseInt((r+"").replace(find, replace));
        occ += StaticStuff.countOccurrences(g+"", find); g = Integer.parseInt((g+"").replace(find, replace));
        occ += StaticStuff.countOccurrences(b+"", find); b = Integer.parseInt((b+"").replace(find, replace));
        return occ;
    }

    private void setColorFromEditor(){
        if(!ColorChooserDemo.visible){ ColorChooserDemo.openChooser(); return;}
