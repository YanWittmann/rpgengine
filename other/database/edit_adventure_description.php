...
//for example: edit_adventure_description.php?user_name=test&password=12345678&adventure_id=0&new_attribute=newattr
	...
	if($is_valid_user) {
		...
		if($user_id_from_name > -1) {
			...
			if($query->num_rows > 0) {
				...
				//edit adventure
				execute_SQL($db, "UPDATE `rpg_adventure` SET `adv_desc` = '" . $new_attribute . "' WHERE `rpg_adventure`.`adventure_id` = " . $adventure_id);
